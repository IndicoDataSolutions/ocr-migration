...
from comparison_helpers import convert_for_file, convert_toolkit
...
if __name__ == "__main__":
    ...
    param_defaults = {
        ...
        "NUM_KEYPOINTS_FOR_ALIGNMENT": None,
        "PARTIAL_AFFINE": True,
        "maxIters": None,
        "refineIters": None,
        "confidence": None,
        "ransacReprojThreshold": None,
        "TEXT_EXTRACTION_ATOL": None,
        "NEWLINE_PIXELS_THRESHOLD": None,
        "granularity": None,
        "engine_pair": None,
    }

    wandb.init(project="ocr_migration", entity="indico", config=param_defaults)
    config = wandb.config

    aligner_config = AlignerConfig(config)
    all_results = run_all_docs(aligner_config)
    new_ocr = pd.read_csv(
        f"./{aligner_config.new_engine_folder_name}/all_labels.csv", index_col=0
    )
    all_label_comparisons = []
    for file_name, res_dict in all_results.items():
        labels_for_file = json.loads(
            new_ocr[new_ocr["file_name"] == file_name]["labels"].values[0]
        )
        all_label_comparisons.append(
            {
                "file": file_name,
                "comparison": convert_for_file(labels_for_file, res_dict),
            }
        )
    ...
    def metric(f1s):
        ...
        spread = (max(f1s) - min(f1s)) / np.mean(f1s)
        return spread

    all_f1 = [
        F1(
            c["comparison"]["overall"]["precision"],
            c["comparison"]["overall"]["recall"],
        )
        for c in all_label_comparisons
    ]
